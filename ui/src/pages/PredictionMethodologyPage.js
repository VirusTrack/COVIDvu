    ...
    return (
            ...
            <ContentLayout>
                ...
                <Content>
                    ...
                    <p>In <em>covidvu.predict</em> we apply the <a href="https://en.wikipedia.org/wiki/Logistic_function" rel="noopener noreferer">logistic equation</a> to model the 
                        ...
                        We represent these dynamics mathematically with the following differential equation:</p>
...
{/* $$\frac{\mathrm{d}X}{\mathrm{d}t} = r X \left(1 - \frac{X}{K} \right)$$ */}
                    ...
                    <ul>
                        <li><MathJax.Node inline formula={'t'} /> = time</li>
                        <li><MathJax.Node inline formula={'X'} /> = population size, analogous to the total number of infected individuals</li>
                        <li><MathJax.Node inline formula={'r'} /> = growth rate, which is the rate at which the virus spread if left unimpeded in an infinite-sized population</li>
                        <li><MathJax.Node inline formula={'K'} /> = carrying capacity, which is the total number of infected individuals as <MathJax.Node inline formula={'t \\rightarrow \\infty'} /> 
                        in a finite-sized population given constraints such as hand washing, social isolation, health-care effectiveness etc. </li>
                    </ul>
                    ...
                    <p>
                        ...
                        where the total number of cases <MathJax.Node inline formula={'X \\leq 50'} />, and require at 
                    ...
                    <p>
                        where <MathJax.Node inline formula={'X_0'} /> is the initial infected population size. Assuming <MathJax.Node inline formula={'K \\gg X_0'} />, we re-cast this equation in the form
                    </p>

                    <MathJax.Node formula={`X(t) = \\frac{L}{1+e^{-k(t-t_0)}}`} />

                    <p>
                        where <MathJax.Node inline formula={`K=L`} />, <MathJax.Node inline formula={`r=k`} />, and <MathJax.Node inline formula={`t_0=1/r \\ln(K)`} />.
                    </p>
                    ...
                    <p>
                        where <MathJax.Node inline formula={`\\mathcal{N}(\\mu, \\sigma^2)`} /> is a normal distribution with mean <MathJax.Node inline formula={`\\mu`} /> and variance <MathJax.Node inline formula={`\\sigma^2`} />. In 
                        other words, we assume that the mean number of cases follows the logistic equation, with Normally-distributed noise in 
                        log space. <MathJax.Node inline formula={`Y_\\theta(t)`} /> denotes that <MathJax.Node inline formula={`Y(t)`} /> is parametrized by <MathJax.Node inline formula={`\\theta`} />. Defining the error 
                    ...
                    <p>
                        We perform Bayesian inference using a No-U-Turn Sampler (NUTS; Hoffman, 2014) as implemented in <a href="https://pystan.readthedocs.io/en/latest/" rel="noopener noreferer">PyStan</a> using 
                    ...
                    <p>We return approximate central Bayesian confidence intervals and the posterior mean for 
                        <MathJax.Node inline formula={'X(t)'} /> in <em>covidvu.predict.predictLogisticGrowth</em>. One may interpret 
                        the e.g. 95% confidence interval with the following statement:</p>
                    
                    <p>"Assuming that the above model is the true model describing the dynamics of the spread of COVID-19 
                        through a particular population, and given our model priors, we may say with approximately 95% confidence 
                        that the true value of <MathJax.Node inline formula={'X(t)'} /> will lie within the confidence bounds".</p>

                    <p>We use the word "approximate" because Hamiltonian Monte Carlo will only guarantee that the numerically-generated 
                        samples from the posterior are true samples in the limit of an infinitely long chain. One may make the approximations 
                        more accurate by increasing nSamples in <em>covidvu.predict.predictLogisticGrowth</em>, and to a lesser extent, nTune and nChains.</p>

                    <p>We also return the posterior mean, which is a good point-estimate for the dynamics of COVID-19 given our uncertainty 
                        in model parameters, given the observed data. </p>
